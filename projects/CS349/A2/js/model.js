...
function createModelModule() {
    ...
    _.extend(ImageModel.prototype, {
        ...
        setCaption: function(caption) {
            ...
            if(!_.isString(caption)){
                throw new Error("Invalid arguments to ImageModel.setCaption: " + JSON.stringify(arguments));
            }
            this.caption=caption;
            _.each(this.listeners, function(listener) {
                listener(self, (new Date()).getTime());
            });
        },
        ...
        setRating: function(rating) {
            // TODO
            var self=this;
            if(!(_.isNumber(rating)&&rating>=0&&rating<=5)){
                throw new Error("Invalid arguments to ImageModel.setCaption: " + JSON.stringify(arguments));
            }
            this.rating=rating;
            _.each(this.listeners, function(listener) {
    ...
    _.extend(ImageCollectionModel.prototype, {
        ...
        addImageModel: function(imageModel) {
            ...
            var self=this;
            addOrRemove(imageModel,this,"Add");
            
            

            
            _.each(this.listeners, function(listener) {
                listener(IMAGE_ADDED_TO_COLLECTION_EVENT,self,imageModel, (new Date()).getTime());
            });
        ...
        removeImageModel: function(imageModel) {
            ...
            addOrRemove(imageModel,this,"Remove");
            this.imageModels = _.without(this.imageModels, imageModel);

            var self=this;
            _.each(this.listeners, function(listener) {
                listener(IMAGE_REMOVED_FROM_COLLECTION_EVENT,self,imageModel, (new Date()).getTime());
            });
        },
    ...
    var getSelf;
    var modelListen= function(imageModelRet,eventTime){
                _.each(getSelf.listeners, function(listener) {
                    listener(IMAGE_META_DATA_CHANGED_EVENT,getSelf,imageModelRet, (new Date()).getTime());
                });
            }

    var addOrRemove = function(imageModel,collection,str){
        
        

        if(str=="Add"){
            getSelf=collection;
            imageModel.addListener(modelListen);
        }
        else{
            getSelf=collection;
            imageModel.removeListener(modelListen);
